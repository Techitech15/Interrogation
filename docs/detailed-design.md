...
| ボリューム | 無料体験版1事件、製品版3事件+隠し1事件 | 資料3 |
...
| SCR-02 | 事件選択 | 事件カード一覧（体験版は1枚、製品版は3+隠し1） | Case[] |
...
// case.json
{
  ...
  "title": "深夜の密室",
  "isFreeDemo": true,
  "detentionLimitMinutes": 2880,        // 48時間 = 2880分（ゲーム内時間単位）
  "briefing": {
...
### P3 製品版（3〜4ヶ月）
- [ ] 事件2〜3件+隠し事件の追加（データ制作、コード変更なしで追加できることを確認）
...
| シナリオテスト（人手） | 事件データ一式 | 全エンディング（自白/釈放/冤罪）に実際に到達できることを確認 |
